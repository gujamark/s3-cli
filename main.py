...
parser.add_argument("-uf",
                    "--upload_file",
                    choices=["False", "True"],
                    help="Upload small file to bucket",
                    type=str,
                    nargs="?",
                    const="True",
                    default="False")

parser.add_argument("-mpu",
                    "--multipart_upload",
                    choices=["False", "True"],
                    help="Multipart file upload",
                    type=str,
                    nargs="?",
                    const="True",
                    default="False")
...
def main():
  ...
  if args.bucket_name:
    # print(s3_client.get_bucket_lifecycle_configuration(Bucket=args.bucket_name))
    ...
    if args.file:
      ...
      if mime.from_file(args.file) in ALLOWED_MIMES:
        if args.upload_file == "True":
          upload_file(s3_client, args.file, args.bucket_name)
        elif args.multipart_upload == "True":
          multipart_upload(s3_client,args.file,args.bucket_name, args.file)
      else:
        print("This file type isn't allowed")

    if args.lifecycle_expiration_days:
      if args.lifecycle_expiration_days <= 0:
        print("lifecycle_expiration_days must be greater than 0")
      else:
        if put_lifecycle_policy(s3_client,args.bucket_name,args.lifecycle_expiration_days):
