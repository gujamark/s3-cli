...
def put_lifecycle_policy(aws_s3_client, bucket_name, expiration_days):
  lc_config = {
    'Rules' : [
      {
        'Expiration' : {
  ...
  response = aws_s3_client.put_bucket_lifecycle_configuration(Bucket=bucket_name, LifecycleConfiguration=lc_config)
